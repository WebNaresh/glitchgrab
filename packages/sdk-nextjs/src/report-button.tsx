...
import type { ReportButtonProps, ReportType, ReportSeverity, UseGlitchgrabReturn } from "./types";
...
export function ReportButton({
  ...
  const handleOpen = async () => {
    setCapturing(true);
    setSubmitted(false);
    // Auto-skip step 1 if only one type is available
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      if (detail?.type) {
        ...
        // Skip step 1 if type is pre-selected in stepper mode
  ...
  const handleSubmit = async () => {
    try {
      ...
      const result = variant === "stepper"
        ? await report(reportType, description.trim(), Object.keys(metadata).length > 0 ? metadata : undefined)
        : await reportBug(description.trim(), Object.keys(metadata).length > 0 ? metadata : undefined);
          ...
          >
            ...
            <div style={{ padding: "16px" }}>
              {submitted ? (
                ...
                >
                  {variant === "stepper" ? `${getTypeLabel(reportType)} sent. Thank you!` : "Bug report sent. Thank you!"}
                </div>
              ) : variant === "stepper" ? (
                /* ─── Stepper Flow ─── */
                <>
